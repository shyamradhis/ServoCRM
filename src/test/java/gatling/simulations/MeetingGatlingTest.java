 ...
 */
public class MeetingGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all meetings").get("/api/meetings").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new meeting")
                        .post("/api/meetings")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created meeting").get("${new_meeting_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created meeting").delete("${new_meeting_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Meeting entity").exec(scn);
