 ...
 */
public class AccountsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all accounts").get("/api/accounts").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new accounts")
                        .post("/api/accounts")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"account_name\": \"SAMPLE_TEXT\"" + ", \"phone\": 0" + ", \"website\": \"SAMPLE_TEXT\"" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created accounts").get("${new_accounts_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created accounts").delete("${new_accounts_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Accounts entity").exec(scn);
