...
public class DealsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all deals").get("/api/deals").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new deals")
                        .post("/api/deals")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"amount\": \"SAMPLE_TEXT\"" +
                                ", \"deal_name\": \"SAMPLE_TEXT\"" +
                                ", \"closing_date\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"account_name\": \"SAMPLE_TEXT\"" +
                                ", \"stage\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_deals_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created deals").get("${new_deals_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created deals").delete("${new_deals_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Deals entity").exec(scn);
